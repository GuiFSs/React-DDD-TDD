...
describe('SurveyResult Component', () => {
  ...
  test('Should present SurveyResult data on success', async () => {
    ...
    const surveyResult: LoadSurveyResult.Model = {
    ...
    const { answers: surveyAnswers } = surveyResult
    expect(images[0]).toHaveAttribute('src', surveyAnswers[0].image)
    expect(images[0]).toHaveAttribute('alt', surveyAnswers[0].answer)
    expect(images[1]).toBeFalsy()

    const answers = screen.queryAllByTestId('answer')
    expect(answers[0]).toHaveTextContent(surveyAnswers[0].answer)
    expect(answers[1]).toHaveTextContent(surveyAnswers[1].answer)

    const percents = screen.queryAllByTestId('percent')
    expect(percents[0]).toHaveTextContent(`${surveyAnswers[0].percent}%`)
    expect(percents[1]).toHaveTextContent(`${surveyAnswers[1].percent}%`)
  })
